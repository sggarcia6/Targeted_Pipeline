...
```{r}
# Create an empty list to store results for each compound
results_list <- list()

# Loop through each compound
for (compound_name in unique(poo_cmpds_BMIS_df$compound_name)) {
  # Extract the compound area from BMIS_df_wide
  compound_area <- poo_cmpds_BMIS_df %>%
    filter(compound_name == compound_name) %>%
    select(-filename, -compound_name) %>%
    t() %>%
    as.numeric()
  
  # Loop through each internal standard and calculate the division
  for (internal_standard in IS) {
    # Extract the internal standard area from BMIS_df_wide
    internal_standard_area <- BMIS_df_wide %>%
      filter(compound_name == internal_standard) %>%
      select(-filename, -compound_name) %>%
      t() %>%
      as.numeric()
    
    # Perform the division
    result <- compound_area / internal_standard_area
    
    # Create a new column with the result
    BMIS_df_wide <- BMIS_df_wide %>%
      mutate(!!paste0("BMIS_", compound_name) := result)
  }
  
  # Store the result for this compound in the results_list
  results_list[[compound_name]] <- BMIS_df_wide %>%
    select(filename, !!paste0("BMIS_", compound_name))
  
  # Remove the intermediate columns used for calculation
  BMIS_df_wide <- BMIS_df_wide %>%
    select(-matches("^BMIS_"))
}

# Create the final BMIS dataframe
BMIS_df <- bind_cols(results_list)

# View the final BMIS dataframe
print(BMIS_df)
